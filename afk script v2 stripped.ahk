...
LoopSettings() {
    ...
    if(activeWindows.Length > 0) {
        ...
        if (accountWithMostActions > 0) {
            ...
            if(mainAccount[4])  totalTime := Max(totalTime, 200)   ; M1 timing
            if(mainAccount[5])  totalTime := Max(totalTime, 200)   ; M2 timing
            if(mainAccount[6])  totalTime := Max(totalTime, 200)   ; Special 1 timing
            if(mainAccount[7])  totalTime := Max(totalTime, 300)   ; Special 2 timing
            if(mainAccount[8])  totalTime := Max(totalTime, AutoRHoldTime)  ; R timing
            if(mainAccount[9])  totalTime := Max(totalTime, AutoTHoldTime)   ; T timing
            if(mainAccount[10]) totalTime := Max(totalTime, AutoLootHoldTime)  ; Loot timing
            
            ; Use minSleepTime if totalTime is lower
            sleepTime := totalTime < minSleepTime ? minSleepTime : totalTime + 500
                    ...
                    if WinExist("Trove - Error Handler") {
                        ...
                        try {
                            if WinExist("Trove") {
                                ...
                                ; Move window if coordinates specified, otherwise default position. if you wish to set it to 0,0,0,0 just put 1 in winX, or uncomment the else statement below
                                if (value[16] != 0) {
                                    x := Integer(value[14])
                                    y := Integer(value[15])
                                    w := Integer(value[16])
